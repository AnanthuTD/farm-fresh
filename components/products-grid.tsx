...
function isCategoryAvailableNow(c?: CategoryItem | null): boolean {
  ...
  const allowedDays =
    c.availableDays && c.availableDays.length > 0 ? c.availableDays : undefined;
  if (allowedDays && !allowedDays.includes(day)) return false;
  if (c.availableTimeStart && c.availableTimeEnd) {
    const [sh, sm] = c.availableTimeStart
      .split(":")
      .map((n) => parseInt(n, 10));
...
export function ProductsGrid() {
  ...
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      {(loading
        ? Array.from({ length: 6 }).map((_, i) => ({ id: `s-${i}` }))
        : filtered
      ).map((product: any) => {
        const cat = categoriesById.get(product.category) || null;
        const availableNow =
          product?.available !== false && isCategoryAvailableNow(cat);
        return (
          <Card
            key={product.id}
            className={`group transition-all duration-300 pt-0 ${
              availableNow ? "hover:shadow-lg" : "opacity-50 grayscale"
            }`}
          >
            <Link
              href={availableNow ? `/products/${product.id}` : "#"}
              className={availableNow ? "" : "pointer-events-none"}
            >
              ...
              <CardContent className="p-4">
                <h3 className="text-lg font-semibold text-foreground mb-2">
                  {loading ? (
                    <span className="inline-block h-5 w-40 bg-muted animate-pulse rounded" />
                  ) : (
                    product.name
                  )}
                </h3>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {!loading && `${product.weight} ${product.weightUnit}`}
                    </span>
                  </div>
                  {!loading && (
                    <Button
...
function InterestButton({ productId }: { productId: string }) {
  const [done, setDone] = useState<boolean>(
    typeof window !== "undefined"
      ? !!localStorage.getItem(`interest_${productId}`)
      : false
  );
  ...
  if (done) {
    return (
      <div className="text-xs text-muted-foreground">
        Thanks! We'll let the admin know.
      </div>
    );
  }
  return (
